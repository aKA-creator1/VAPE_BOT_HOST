...
def load_products():
    if os.path.exists(PRODUCTS_FILE):
        try:
            ...
            logger.error("Ошибка чтения products.json, создаю дефолтные товары")
    ...
    default_products = [
        {
            ...
            "available": True, 
            "variants": [
                {"name": "Синий", "price": 2490},
                {"name": "Черный", "price": 2490}
            ], 
        ...
        },
        {
            ...
            "category": "Жидкости", 
            "subcategory": "liquid", 
            "description": "Солевая жидкость 20mg", 
            "image": "/product_images/default.png", 
            "available": True, 
            "variants": [
...
def load_orders():
    if os.path.exists(ORDERS_FILE):
        try:
            ...
            logger.error("Ошибка чтения orders.json")
    return []
...
async def send_order_to_admins(order):
    try:
        ...
        logger.error(f"Ошибка в send_order_to_admins: {e}")
...
async def order_management(callback: types.CallbackQuery):
    ...
    try:
        ...
        if action in status_map:
            ...
            emoji = "✅" if action == "confirm" else "❌" if action == "cancel" else "🚚"
            await callback.answer(f"{emoji} Заказ {status_map[action].lower()}!")
        ...
        logger.error(f"Ошибка в order_management: {e}")
...
async def menu_command(message: types.Message):
    ...
    await message.answer("Нажмите на кнопку, чтобы открыть витрину:", reply_markup=keyboard)
...
async def handle_api(request):
    try:
        ...
        return web.Response(text=json.dumps({"error": str(e)}), content_type='application/json')
...
async def handle_order(request):
    try:
        ...
        if not data.get('user_id') or not data.get('items'):
            return web.Response(
                text=json.dumps({"status": "error", "message": "Недостаточно данных"}),
                content_type='application/json',
                headers={"Access-Control-Allow-Origin": "*"},
                status=400
            )
        ...
        return web.Response(
            text=json.dumps({"status": "success", "order_id": order["id"]}),
            content_type='application/json',
            headers={"Access-Control-Allow-Origin": "*"}
        )
    except Exception as e:
...
async def handle_user_orders(request):
    try:
        ...
        if not user_id:
            return web.Response(
                text=json.dumps({"status": "error", "message": "Не указан user_id"}),
                content_type='application/json',
                headers={"Access-Control-Allow-Origin": "*"},
                status=400
            )
        ...
        return web.Response(
            text=json.dumps({"orders": user_orders}, ensure_ascii=False),
            content_type='application/json',
            headers={"Access-Control-Allow-Origin": "*"}
        )
    except Exception as e:
...
async def handle_cancel_order(request):
    try:
        ...
        if not order:
            return web.Response(
                text=json.dumps({"status": "error", "message": "Заказ не найден"}),
                content_type='application/json',
                headers={"Access-Control-Allow-Origin": "*"},
                status=404
            )
        ...
        if order["user_id"] != user_id:
            return web.Response(
                text=json.dumps({"status": "error", "message": "Это не ваш заказ"}),
                content_type='application/json',
                headers={"Access-Control-Allow-Origin": "*"},
                status=403
            )
        ...
        if order["status"] in ["Доставлен", "Отменён"]:
            return web.Response(
                text=json.dumps({"status": "error", "message": f"Заказ уже {order['status'].lower()}"}),
                content_type='application/json',
                headers={"Access-Control-Allow-Origin": "*"},
                status=400
            )
        ...
        for admin_id in ADMIN_IDS:
            try:
                await bot.send_message(
                    admin_id, 
                    f"❌ <b>ЗАКАЗ #{order_id} ОТМЕНЁН ПОКУПАТЕЛЕМ</b>\n\n👤 {order['user_name']}\n💰 {order['total']} ₽\n📅 {datetime.now().strftime('%d.%m.%Y %H:%M')}",
                    parse_mode="HTML"
                )
            except Exception as e:
        ...
        try:
            await bot.send_message(
                user_id, 
                f"❌ <b>Ваш заказ #{order_id} отменён</b>\n\nЕсли вы хотите его восстановить, свяжитесь с менеджером.",
                parse_mode="HTML"
            )
        except Exception as e:
...
async def admin_api(request):
    try:
        # Обработка multipart/form-data (загрузка изображений)
        ...
        if action == 'get_products':
            ...
            return web.Response(
                text=json.dumps({"status": "success", "product": product}),
                content_type='application/json',
                headers={"Access-Control-Allow-Origin": "*"}
            )
            ...
            return web.Response(
                text=json.dumps({"status": "error", "message": "Unknown action"}),
                content_type='application/json',
                headers={"Access-Control-Allow-Origin": "*"},
                status=400
            )
        ...
        logger.error(f"Ошибка админ API: {e}")
        return web.Response(
            text=json.dumps({"status": "error", "message": str(e)}),
            content_type='application/json',
            headers={"Access-Control-Allow-Origin": "*"},
...
async def index_handler(request):
    try:
        ...
        return web.Response(
            text="<h1>Файл index.html не найден</h1>",
...
async def admin_html_handler(request):
    try:
        ...
        return web.Response(
            text="<h1>Файл admin.html не найден</h1>",
            content_type='text/html'
        )
